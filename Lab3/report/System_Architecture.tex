This is a digital audio recording and playback system implemented on FPGA (DE2-115 development board). The system uses WM8731 audio codec for analog-to-digital conversion and SRAM as audio data buffer. The system supports recording, playback, variable speed playback (fast/slow), and various audio effects processing.
...
\textbf{Function}: Connects FPGA with DE2-115 development board hardware resources

\textbf{Main Responsibilities}:
\begin{itemize}
    \item Button debouncing (\texttt{Debounce})
    \item PLL clock generation (\texttt{Altpll})
    \begin{itemize}
...
- SRAM_ADDR[19:0], SRAM_DQ[15:0]: SRAM interface
...
\begin{itemize}
    ...
    \item Clean mode: \texttt{addr\_clean} → SRAM\_ADDR, \texttt{data\_clean} → SRAM\_DQ
\end{itemize}
...
\textbf{Function}: Handle address control, speed adjustment, interpolation, and audio effects processing during playback

\textbf{Core Functions}:

\paragraph{Playback Control State Machine}
\begin{itemize}
...
\paragraph{Fast Playback Mode (\texttt{i\_fast=1})}
\begin{itemize}
...
\paragraph{Slow Playback Mode (\texttt{i\_slow\_0} or \texttt{i\_slow\_1})}
\begin{itemize}
...
\paragraph{Robot Voice Effect (\texttt{i\_voice\_robot=1})}
\begin{itemize}
...
\textbf{Data Flow}:
\begin{verbatim}
SRAM → i_sram_data → Speed/Interpolation Processing → Robot Effect Processing → o_dac_data
\end{verbatim}

\subsubsection{AudPlayer.sv (Playback Module)}

\textbf{Function}: Send DSP-processed audio data to WM8731 DAC through I2S protocol
...
\begin{itemize}
    ...
    \item Dynamic update: Monitor \texttt{i\_mode} and address changes, update only changed parts
\end{itemize}
...
\begin{verbatim}
WM8731 ADC → I2S (ADCDAT)
    ↓
AudRecorder (Serial→Parallel Conversion)
    ↓
SRAM (Write, Address Increment)
    ↓
Record Maximum Address (record_max_addr_r)
\end{verbatim}

\subsubsection{Playback Flow}
\begin{verbatim}
SRAM (Read, Address Controlled by AudDSP)
    ↓
AudDSP (Speed Control, Interpolation, Audio Effects Processing)
    ↓
AudPlayer (Parallel→Serial Conversion)
    ↓
WM8731 DAC → I2S (DACDAT)
    ↓
Analog Audio Output
\end{verbatim}
...
\begin{itemize}
    ...
    \item \textbf{Maximum Recording Time}: Approximately 30 seconds (32 kHz sample rate)
\end{itemize}
