The system uses a hierarchical finite state machine (FSM) structure to coordinate hardware operations across multiple modules. The main FSM in \texttt{Top.sv} controls overall system operation, while sub-modules implement their own FSMs for specific tasks.
...
\begin{tikzpicture}[
    ...
    auto, 
    thick,
    state/.style={rectangle, rounded corners, draw=black, fill=gray!20, minimum width=2cm, minimum height=1cm, text centered},
    initial/.style={fill=green!20}
]
    % Define states
    \node[state, initial] (IDLE) {IDLE};
    \node[state, right of=IDLE, xshift=1cm] (I2C) {I2C};
    \node[state, below of=I2C] (CLEAN) {CLEAN};
    \node[state, left of=CLEAN, xshift=-1cm] (RECD) {RECD};
    \node[state, below of=RECD] (RECDPAUSE) {RECD\_PAUSE};
    \node[state, right of=RECD, xshift=1cm] (PLAY) {PLAY};
    \node[state, below of=PLAY] (PLAYPAUSE) {PLAY\_PAUSE};
    
    % Define transitions
    \path[->] (IDLE) edge[bend left=20] node[above] {Boot seq} (I2C)
              (I2C) edge node[right] {Init done} (CLEAN)
              (CLEAN) edge node[left] {Clean done} (IDLE)
              (IDLE) edge[bend right=30] node[left] {KEY1} (RECD)
              (IDLE) edge[bend left=30] node[right] {KEY0} (PLAY)
              (RECD) edge node[left] {KEY1} (RECDPAUSE)
              (RECDPAUSE) edge node[right] {KEY1} (RECD)
              (RECD) edge node[above, sloped] {KEY2} (IDLE)
              (RECDPAUSE) edge node[below, sloped] {KEY2} (IDLE)
              (PLAY) edge node[right] {KEY0} (PLAYPAUSE)
              (PLAYPAUSE) edge node[left] {KEY0} (PLAY)
              (PLAY) edge[bend right=30] node[above, sloped] {KEY2 or end} (IDLE)
              (PLAYPAUSE) edge node[below, sloped] {KEY2} (IDLE);
\end{tikzpicture}
...
\textbf{Key Operations in S\_PLAY State}:
\begin{itemize}
...
\textbf{Key Operations in RECORDING State}:
\begin{itemize}
